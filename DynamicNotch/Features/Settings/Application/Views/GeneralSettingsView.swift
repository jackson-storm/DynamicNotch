...
struct GeneralSettingsView: View {
    ...
    private var themeCard: some View {
        SettingsCard(title: "Appearance") {
            ...
            CustomPicker(
                selection: $applicationSettings.windowStyle,
                options: Array(SettingsWindowStyle.allCases),
                title: { $0.title },
                headerTitle: "settings.general.windowStyle.title",
                headerDescription: "settings.general.windowStyle.description",
                itemHeight: 110,
                lightBackgroundImage: Image("backgroundLight"),
                darkBackgroundImage: Image("backgroundDark")
            ) { style, isSelected in
                WindowStylePickerContent(style: style, isSelected: isSelected)
            }
            .accessibilityIdentifier("settings.general.windowStyle")
        }
    }
...
private struct WindowStylePickerContent: View {
    @Environment(\.colorScheme) private var colorScheme
    ...
    var body: some View {
        WindowStyleMiniWindow(
            style: colorScheme == .dark ? .dark : .light,
            isTranslucent: style == .semiTranslucent
        )
        .frame(width: 110, height: 80)
    }
}

private struct WindowStyleMiniWindow: View {
    typealias Style = ThemeMiniWindow.Style
    ...
    var body: some View {
        ...
        let backgroundStyle: AnyShapeStyle = {
            if isTranslucent {
                if style == .dark {
                    return AnyShapeStyle(.ultraThinMaterial.opacity(0.35))
                } else {
                    return AnyShapeStyle(.thickMaterial.opacity(0.6))
                }
            } else {
                return AnyShapeStyle(style.background)
            }
        }()
    ...
    private var chromeBar: some View {
        ...
        .background {
            if isTranslucent {
                AnyView(Color.clear.background(.thinMaterial.opacity(0.3)))
            } else {
                AnyView(style.chrome)
            }
        }
    }
    ...
    private var sidebar: some View {
        ...
        .background {
            if isTranslucent {
                AnyView(Color.clear.background(.ultraThinMaterial.opacity(0.2)))
            } else {
                AnyView(style.sidebar)
            }
        }
    }
