...
final class NotchViewModelIntegrationTests: XCTestCase {
    ...
    @MainActor
    func testNotchShapeStyleNotchUsesStandardNotchSizeAndDisablesDynamicIsland() async {
        let settings = TestNotchSettings(notchShapeStyle: .notch)
        let viewModel = NotchViewModel(
            settings: settings,
            hideDelay: 0.01,
            queueDelay: 0,
        ...
        XCTAssertFalse(viewModel.notchModel.isDynamicIsland)
        XCTAssertEqual(viewModel.notchModel.baseWidth, 160, accuracy: 0.001)
        XCTAssertEqual(viewModel.notchModel.baseHeight, 30, accuracy: 0.001)

        // Verify content sizing uses DynamicIslandCustomizable
        let content = HudNotchContent(kind: .volume, level: 50)
        viewModel.send(.showLiveActivity(content))

        await assertEventually {
            await MainActor.run { viewModel.notchModel.liveActivityContent?.id == content.id }
        }

        let expectedSize = content.dynamicIslandSize(baseWidth: 160, baseHeight: 30)
        XCTAssertEqual(viewModel.notchModel.size.width, expectedSize.width, accuracy: 0.001)
        XCTAssertEqual(viewModel.notchModel.size.height, expectedSize.height, accuracy: 0.001)
    }

    @MainActor
    func testNotchShapeStyleCapsuleUsesDynamicIslandSize() async {
        let settings = TestNotchSettings(notchShapeStyle: .capsule)
        let viewModel = NotchViewModel(
            settings: settings,
            hideDelay: 0.01,
            queueDelay: 0,
        ...
        XCTAssertTrue(viewModel.notchModel.isDynamicIsland)
        XCTAssertEqual(viewModel.notchModel.baseWidth, 120 * 0.85, accuracy: 0.001)
        XCTAssertEqual(viewModel.notchModel.baseHeight, 25, accuracy: 0.001)
        ...
        let expectedIslandSize = content.dynamicIslandSize(baseWidth: 120 * 0.85, baseHeight: 25)
        XCTAssertEqual(viewModel.notchModel.size.width, expectedIslandSize.width, accuracy: 0.001)
        XCTAssertEqual(viewModel.notchModel.size.height, expectedIslandSize.height, accuracy: 0.001)
    }
}
